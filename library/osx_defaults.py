...
class OSXDefaults(object):
    ...
    def run(self):
        ...
        # There is a type mismatch! Given type does not match the type in defaults, ignoring dict
        if self.current_value is not None and type(self.current_value) is not dict and type(self.current_value) is not type(self.value):
